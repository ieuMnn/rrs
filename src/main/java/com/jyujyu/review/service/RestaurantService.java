...
@Service // 사용자요청에 따른 DB 데이터 추/삭/수
public class RestaurantService {

    private final RestaurantRepository restaurantRepository;
    private final MenuRepository menuRepository;

    @Transactional // DB에 트랜잭션을 열고 닫음.
    public RestaurantEntity createRestaurant(
            CreateAndEditRestaurantRequest request
    ){
//        RestaurantEntity restaurant = new RestaurantEntity();// 이거 말고 아래 빌더 사용.
    ...
    @Transactional(readOnly = true)
    public RestaurantDetailView getRestaurantDatail(Long restaurandId){
        RestaurantEntity restaurant = restaurantRepository.findById(restaurandId).orElseThrow();
        List<MenuEntity> menus=menuRepository.findAllByRestaurantId(restaurandId);
